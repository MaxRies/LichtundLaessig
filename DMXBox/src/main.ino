...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Funtions-Aufruf des Devices

  if (strcmp(subTopic, "/DMX_kill")==0){    // strcmp liefert 0 wenn strings identisch
    dev.kill();
  }
  else if (strcmp(subTopic, "/DMX_update") == 0){
    dev.update(msg);
  }
  else if (strcmp(subTopic, "/DMX_init") == 0){
    dev.setup(msg.toInt());
  }
}
